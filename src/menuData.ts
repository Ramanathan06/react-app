...
export interface MenuItem {
  id: number;
  name: string;
  category: 'starter' | 'main' | 'sides' | 'desert';
  isVeg: boolean;
  description: string;
...
export const menuData: MenuItem[] = [
  ...
  },
  {
    ...
    "category": "starter",
    "isVeg": true,
    "description": "Spinach and green pea patties shallow-fried until crispy outside.",
...
export interface FilterParams {
  category?: 'starter' | 'main' | 'sides' | 'desert' | 'all';
  diet?: 'veg' | 'nonveg' | 'all';
  name?: string;
}

export function filterMenuItems(params: FilterParams): MenuItem[] {
  return menuData.filter((item) => {
    ...
    if (params.diet && params.diet !== 'all') {
      const isVegNeeded = params.diet === 'veg';
      if (item.isVeg !== isVegNeeded) {
        return false;
      }
    }
    if (params.name && params.name.trim() !== '') {
      const searchStr = params.name.toLowerCase();
